...
select id_zam, datediff(curdate(), data_zam) as 'Dni od zamówienia' 
...
select id_zam, id_klienta, id_dostawcy, forma_zaplaty 
from czek.zamowienia 
...
select imie, nazwisko, miasto, kod 
from czek.klienci 
where kod like '70%' or kod like '1____1';
...
select max(cena_pudelka) as 'Maksymalna cena pudełka', min(cena_pudelka) as 'Minimalna cena pudełka', avg(cena_pudelka) as 'Średnia cena pudełka' 
from czek.pudelka;
...
select region, count(*) as 'Liczba klientów' 
from czek.klienci 
group by region;

h)identyfikatory klientów i liczbę złożonych przez nich zamówień.
select id_klienta, count(*) as 'Liczba zamówień' 
from czek.zamowienia 
group by id_klienta;