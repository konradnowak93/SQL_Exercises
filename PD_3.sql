    1.Napisz w języku SQL zapytania wyświetlające następujące informacje z bazy danych czek:
		...
		select miasto, count(id_klienta) as 'Liczba klientów' 
		from czek.klienci 
		group by miasto 
		...
		from czek.zamowienia z join czek.klienci k on z.id_klienta = k.id_klienta 
		...
		select z.id_zam, z.data_zam, k.imie as 'Imię klienta', k.nazwisko as 'Nazwisko klienta', d.imie as 'Imię dostawcy', d.nazwisko as 'Nazwisko dostawcy' 
		from czek.zamowienia z join czek.klienci k on z.id_klienta = k.id_klienta join czek.dostawcy d on z.id_dostawcy = d.id_dostawcy 
		order by id_zam ASC;
		...
		from czek.zamowienia group by data_zam 
        ...
        select p.nazwa as 'Nazwa pudełka', c.nazwa as 'Nazwa czekoladki', op.l_sztuk as 'Liczba sztuk czekoladek w pudełku' 
		from czek.pudelka p join czek.o_pudelkach op on p.id_pudelka = op.id_pudelka join czek.czekoladki c on op.id_czek = c.id_czek
		order by p.nazwa, c.nazwa;
		
		g) dla każdego identyfikatora pudełka podaj łączną liczbę (sumę) czekoladek w pudełku,
		select id_pudelka, sum(l_sztuk) 
		...
		select count(id_zam), sposob_zam 
