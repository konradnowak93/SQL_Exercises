Napisz w języku SQL zapytania wyświetlające następujące informacje z bazy danych czek:
	...
	select p.nazwa, 
		count(sz.id_zam) as 'Liczba zamówień' 
	from czek.pudelka p 
		join czek.szczegolowe_zam sz 
			on p.id_pudelka = sz.id_pudelka 
	group by p.nazwa;
	...
	select d.imie, 
		d.nazwisko, 
		count(z.id_zam) as 'Liczba obsłużonych zamówień' 
	from czek.dostawcy d 
		join czek.zamowienia z 
			on d.id_dostawcy = z.id_dostawcy 
	...
	select k.imie, 
		k.nazwisko, 
		count(z.id_zam) as 'Liczba złożonych zamówień'
	from czek.klienci k left 
		join czek.zamowienia z 
	...
	select p.nazwa as 'Nazwa pudełka', 
		cz.nazwa as 'Nazwa czekoladek znajdujących się w pudełku'
	from czek.pudelka p 
		left join czek.o_pudelkach op 
			on p.id_pudelka = op.id_pudelka 
		left join czek.czekoladki cz 
			on op.id_czek = cz.id_czek
	...
	select k.id_klienta, 
		k.imie, 
		k.nazwisko, 
		count(z.id_zam) as 'Liczba zamówien'
	from czek.klienci k
		left join czek.zamowienia z 
			on k.id_klienta = z.id_klienta
	group by k.id_klienta
	...
	from czek.zamowienia z 
		join czek.szczegolowe_zam sz 
			on z.id_zam = sz.id_zam 
		join czek.pudelka p 
			on czek.sz.id_pudelka = p.id_pudelka 
	...
	select nazwa as 'Nazwa czekoladek', 
		koszt as 'Koszt najdroższych czekoladek danego rodzaju', 
		rodzaj_czekolady as 'Rodzaj czekolady'
