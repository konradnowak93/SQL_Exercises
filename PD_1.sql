Napisz w języku SQL zapytania wyświetlające następujące informacje z bazy danych czek:
	...
	select nazwa, 
		...
		sztuk_w_magazynie 
	from czek.pudelka 
	where cena_pudelka > 30 
		or sztuk_w_magazynie >= 500;
	...
	select nazwa, rodzaj_czekolady, 
		rodzaj_nadzienia 
	from czek.czekoladki 
	where rodzaj_nadzienia = "Marcepan" 
		and (rodzaj_czekolady in ("Biała", "Mleczna"));
	/*or*/
	select nazwa, rodzaj_czekolady, 
		rodzaj_nadzienia 
	from czek.czekoladki 
	where rodzaj_nadzienia = "Marcepan" 
		and (rodzaj_czekolady = "Biała" or rodzaj_czekolady = "Mleczna");
	...
	select id_czek, 
		nazwa, 
		koszt, 
		koszt*1.2 as "Koszt po 20% podwyżki" 
	from czek.czekoladki;